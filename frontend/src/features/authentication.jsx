...
export function useAuthentication() {
    ...
    return apiLogin(email, password)
      .then((result) => {
        if (result.status == 200) {
          ...
          localStorage.setItem("authenticationKey", result.authentication_key);
          // Fetch logged in user from backend
          return getByAuthenticationKey(result.authentication_key).then(
            (user) => {
              setAuthenticatedUser(user);
  ...
  async function refresh() {
    if (authenticatedUser) {
      return getByAuthenticationKey(authenticatedUser.authentication_key).then(
        (user) => {
          setAuthenticatedUser(user);
